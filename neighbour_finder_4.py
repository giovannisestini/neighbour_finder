        ...
        if typ_analysis == 0 and random_shuffle == 0: #salI box analysis without randomization
            ...
            for x in range (0, len(random_list)) : # Just the portion of the barcode corresponding to SNPs is written in the temporary file
                ...
                if value == 8 : # Up to 8 SNPs are written on the same line before newline
                    ...
                    value = 0
            out_file.write('\n')
        elif typ_analysis == 2 and random_shuffle == 0 :
            for element in f :
                x = element.split('\t')
                for sal in x :
                    y = sal.split('(')
...
parser.add_argument('-a', '--analysis', help = 'Type of analysis: 0 = SalI boxes, 1 = promoters + all SNPs, 2 = AvaI + CAT, 3 = total repeat, 4 = aAvaI + CAT + 3ETS, 5 = AvaI + 3ETS, 6 = CAT + 3ETS', type = int, required = True)
...
out_combi_file_write = open(out_combi_file, 'w')
